...
// Invoke after starting this worker: resonate invoke --server http://127.0.0.1:8001 --target poll://any@example-durable-chatbot-ts --func processTurn --json-args '[{"history":["hello"],"turnKey":"turn-1","isCrashTurn":false}]' example-durable-chatbot-ts-demo
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(Config.withDefault("example-durable-chatbot-ts"));
