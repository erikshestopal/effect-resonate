  ...
  App.of({
    [functionName]: (input) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run({
            effect: Effect.logInfo(`processed ${input.records.length} records`).pipe(
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(Config.withDefault("example-batch-processor-ts"));
    const pidName = yield* Config.string("RESONATE_PID").pipe(Config.withDefault("example-batch-processor-ts-worker"));
