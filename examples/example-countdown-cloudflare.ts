...
// Invoke after starting this worker: resonate invoke --server http://127.0.0.1:8001 --target poll://any@example-countdown-cloudflare-ts --func countdown --json-args '[2,1]' example-countdown-cloudflare-ts-demo
  ...
  App.of({
    [functionName]: (count, delay) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run({
            effect: Effect.logInfo(`cloudflare countdown ${count}`).pipe(Effect.as(`cloudflare countdown ${count}`)),
          }),
        );
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(
      Config.withDefault("example-countdown-cloudflare-ts"),
    );
    const pidName = yield* Config.string("RESONATE_PID").pipe(
      Config.withDefault("example-countdown-cloudflare-ts-worker"),
    );
