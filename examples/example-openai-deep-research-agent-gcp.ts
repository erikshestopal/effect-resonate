...
// Invoke after starting this worker: resonate invoke --server http://127.0.0.1:8001 --target poll://any@example-openai-deep-research-agent-gcp-ts --func research --json-args '[{"topic":"resonate"}]' example-openai-deep-research-agent-gcp-ts-demo
  ...
  App.of({
    [functionName]: (input) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(Effect.logInfo(`gcp research ${input.topic}`).pipe(Effect.as(`gcp research ${input.topic}`))),
        );
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(
      Config.withDefault("example-openai-deep-research-agent-gcp-ts"),
    );
    const pidName = yield* Config.string("RESONATE_PID").pipe(
      Config.withDefault("example-openai-deep-research-agent-gcp-ts-worker"),
    );
