...
const group = Protocol.WorkerGroup.make(process.env.RESONATE_GROUP ?? "example-openai-deep-research-agent-supabase-ts");
const pid = Protocol.ProcessId.make(
  process.env.RESONATE_PID ?? "example-openai-deep-research-agent-supabase-ts-worker",
);
  ...
  App.of({
    [functionName]: (input) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(
            Effect.logInfo(`supabase research ${input.topic}`).pipe(Effect.as(`supabase research ${input.topic}`)),
          ),
        );
