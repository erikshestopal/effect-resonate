...
export const repo = "example-countdown-web-ts";
  ...
  App.of({
    [functionName]: (count, delay) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(Effect.logInfo(`web countdown ${count}`).pipe(Effect.as(`web countdown ${count}`))),
        );
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(Config.withDefault("example-countdown-web-ts"));
    const pidName = yield* Config.string("RESONATE_PID").pipe(Config.withDefault("example-countdown-web-ts-worker"));
