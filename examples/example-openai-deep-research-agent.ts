...
// Invoke after starting this worker: resonate invoke --server http://127.0.0.1:8001 --target poll://any@example-openai-deep-research-agent-ts --func research --json-args '[{"topic":"resonate"}]' example-openai-deep-research-agent-ts-demo
  ...
  App.of({
    [functionName]: (input) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(
            Effect.logInfo(`plan research ${input.topic}`).pipe(Effect.as(`plan research ${input.topic}`)),
          ),
        );
        results.push(
          yield* ctx.run(Effect.logInfo(`write report ${input.topic}`).pipe(Effect.as(`write report ${input.topic}`))),
        );
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(
      Config.withDefault("example-openai-deep-research-agent-ts"),
    );
    const pidName = yield* Config.string("RESONATE_PID").pipe(
      Config.withDefault("example-openai-deep-research-agent-ts-worker"),
    );
