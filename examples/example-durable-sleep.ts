  ...
  App.of({
    [functionName]: (input) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, Resonate.Context> {
        ...
        results.push(
          yield* ctx.run({
            effect: Effect.logInfo(`Sleeping for ${input} milliseconds`).pipe(
              Effect.as(`Sleeping for ${input} milliseconds`),
            ),
          }),
        );
        results.push(
          yield* ctx.run({
            effect: Effect.logInfo(`Slept for ${input / 1000} seconds`).pipe(
              Effect.as(`Slept for ${input / 1000} seconds`),
            ),
          }),
        );
