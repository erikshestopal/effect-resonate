...
const group = Protocol.WorkerGroup.make(process.env.RESONATE_GROUP ?? "example-countdown-supabase-ts");
const pid = Protocol.ProcessId.make(process.env.RESONATE_PID ?? "example-countdown-supabase-ts-worker");
  ...
  App.of({
    [functionName]: (count, delay) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(Effect.logInfo(`supabase countdown ${count}`).pipe(Effect.as(`supabase countdown ${count}`))),
        );
