...
export const repo = "example-countdown-gcp-ts";
export const functionName = "countdown";
export const sampleArgs = [2, 1] as const;
// Invoke after starting this worker: resonate invoke --server http://127.0.0.1:8001 --target poll://any@example-countdown-gcp-ts --func countdown --json-args '[2,1]' example-countdown-gcp-ts-demo
  ...
  App.of({
    [functionName]: (count, delay) =>
      Effect.gen(function* (): Effect.fn.Return<unknown, unknown, ResonateContext.ResonateContext> {
        ...
        results.push(
          yield* ctx.run(Effect.logInfo(`gcp countdown ${count}`).pipe(Effect.as(`gcp countdown ${count}`))),
        );
...
const worker = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const groupName = yield* Config.string("RESONATE_GROUP").pipe(Config.withDefault("example-countdown-gcp-ts"));
    const pidName = yield* Config.string("RESONATE_PID").pipe(Config.withDefault("example-countdown-gcp-ts-worker"));
