...
| Child timeoutAt clamped to parent's (absolute epoch-ms); detached unclamped                            | 12, 16     | `test/ExecutionEngine.test.ts` local child create path (detached remains spec 16)                                               | partial |
...
| Tag vocabulary: `resonate:target/origin/parent/branch/prefix/scope/timer` (TS/Rust consensus)          | 01, 11, 12 | `test/Protocol.test.ts` "tags"; `test/ResonateClient.test.ts` root target/lineage emission                                      | partial |
| Sleep = `resonate:timer:"true"`, timeoutAt = wake time, no target                                      | 15         |                                                                                                                                 | todo    |
| Deterministic child ids: per-invocation sequence counter (`{parent}.{n}`; detached `{prefix}.d{hash}`) | 12         | `test/ExecutionEngine.test.ts` local `ctx.run` child id                                                                         | partial |
| Structured concurrency: root does not fulfill while attached children unsettled                        | 12, 16     | `test/ExecutionEngine.test.ts` drains local children before root fulfill; remote children remain spec 16                        | partial |
