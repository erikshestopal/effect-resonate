...
| -------------------------------------------------------------------------------------- | --------------------------------------- | --------- | ---------------------------------------------------------------------------------------- | ------ |
| P-01 promise.get (incl. timeout projection)                                            | `spec/02-actions/P-01-promise.get.lean` | 04, 08    | `test/NetworkLocal.test.ts` (oracle side); `test/ProtocolClient.test.ts` DurablePromises | done   |
| P-02 promise.create (new / born-settled / idempotent re-create / target→task / delay)  | `P-02-promise.create.lean`              | 04, 08    | `test/NetworkLocal.test.ts` "P-02"; `test/ProtocolClient.test.ts` DurablePromises        | done   |
| P-03 promise.settle (settle / projected-timeout race / idempotent re-settle / cascade) | `P-03-promise.settle.lean`              | 04, 08    | `test/NetworkLocal.test.ts` "P-03"; `test/ProtocolClient.test.ts` awaitSettled           | done   |
| P-04 promise.register_callback (422 non-target awaiter; skip-if-awaiter-expired)       | `P-04-promise.register_callback.lean`   | 04, 08    | `test/NetworkLocal.test.ts` "P-04"; typed client method in `DurablePromises`             | done   |
| P-05 promise.register_listener                                                         | `P-05-promise.register_listener.lean`   | 04, 08    | `test/NetworkLocal.test.ts` "P-05"; `test/ProtocolClient.test.ts` awaitSettled           | done   |
...
| Spec action                                                                           | Lean source                | Impl spec  | Tests                                                                                                                              | Status |
| ------------------------------------------------------------------------------------- | -------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------ |
| T-01 task.get (projection to fulfilled)                                               | `T-01-task.get.lean`       | 05, 08     | `test/NetworkLocal.test.ts` "T-01…T-10"; typed client method in `Tasks`                                                            | done   |
| T-02 task.create (create+acquire / born-fulfilled / re-acquire / 422 no-target / 409) | `T-02-task.create.lean`    | 05, 08, 11 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks; `test/ResonateClient.test.ts` beginRun root create   | done   |
| T-03 task.acquire (version fence, bump, lease arm)                                    | `T-03-task.acquire.lean`   | 05, 08, 13 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks; `test/Worker.test.ts` execute acquire                | done   |
| T-04 task.fence (fenced create/settle)                                                | `T-04-task.fence.lean`     | 05, 08, 12 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks; `test/ExecutionEngine.test.ts` local steps           | done   |
| T-05 task.heartbeat (bulk, silent per-task no-op, stored-ttl extension)               | `T-05-task.heartbeat.lean` | 05, 08, 13 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks                                                       | done   |
...
| T-07 task.fulfill (atomic settle+fulfill, cascade)                                    | `T-07-task.fulfill.lean`   | 05, 08, 13 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` TaskFenced; `test/Worker.test.ts` root fulfillment          | done   |
| T-08 task.release (→pending, retry timeout, redispatch; resumes NOT cleared)          | `T-08-task.release.lean`   | 05, 08, 13 | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks; worker failure path in `src/Worker.ts`               | done   |
| T-09 task.halt (no version fence; 409 if fulfilled; idempotent)                       | `T-09-task.halt.lean`      | 05, 08     | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks                                                       | done   |
| T-10 task.continue (409 unless halted; no version bump)                               | `T-10-task.continue.lean`  | 05, 08     | `test/NetworkLocal.test.ts` "T-01…T-10"; `test/ProtocolClient.test.ts` Tasks                                                       | done   |
...
| Headers always accompany data; `resonate:schema` header written                                         | 02, 11     | `test/Codec.test.ts` (headers on every encode); `test/ResonateClient.test.ts` invocation params                                                                         | done   |
...
| `409` = stop claiming, never blind-retry; auth failures never retried                                   | 07, 08, 13 | `test/NetworkHttp.test.ts` auth terminal; `test/ProtocolClient.test.ts` TaskFenced                                                                                      | done   |
...
| Seeded deterministic simulation preserves outcomes and invariants under worker restarts/clock chaos     | 22         | `test/DstSimulator.test.ts` fixed seed workflow corpus, exactly-once counters, and raw protocol op-stream fuzzer; `vp run dst`                                          | done   |
| Promise graph shape matches native id/tag/param conventions and remains stable after replay             | 24         | `test/GraphParity.test.ts` local graph walker plus native tree parity gate (`[GRAPH PARITY SKIPPED]` when `resonate` is unavailable)                                    | done   |
| Shipped-server E2E quickstart/interop gate exists and runs when `resonate` CLI is installed             | 23         | `test/E2EInterop.test.ts` (`[E2E SKIPPED]` in this orb because the CLI is unavailable); `vp run e2e`; examples under `examples/`                                        | done   |
