...
| 16  | [remote-invocation](specs/16-remote-invocation.md)             | in-progress | Attached `ctx.beginRpc`/`ctx.rpc` create targeted global child promises and suspend through spec 14 await path; detached/flush/fan-out/explicit-id lineage still pending.                                 |
