...
| 23  | [e2e-interop](specs/23-e2e-interop.md)                         | done   | Server-gated `test/E2EInterop.test.ts` starts `resonate dev` when CLI exists and runs quickstart/interop smoke; examples added. In this orb it skips loudly because the CLI is unavailable.               |
