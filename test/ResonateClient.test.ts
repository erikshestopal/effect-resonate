  ...
  it.effect("beginRpc creates a target-tagged promise and handle await decodes the value", () =>
    Effect.gen(function* () {
      ...
      const handle = yield* client.beginRpc({
        ...
        executionId: Protocol.ExecutionId.make("rpc-1"),
        args: [{ id: "order-1" }],
        options: { target: Protocol.WorkerGroup.make("payments") },
      });
  ...
  it.effect("string-name rpc encodes raw positional args with default version", () =>
    Effect.gen(function* () {
      ...
      const handle = yield* client.beginRpc({
        ...
        executionId: Protocol.ExecutionId.make("string-rpc-1"),
        args: [1, "two"],
      });
  ...
  it.effect("persists retry policy in invocation params", () =>
    Effect.gen(function* () {
      ...
      const handle = yield* client.beginRun({
        ...
        args: [{ id: "order-4" }],
        options: { retryPolicy: RetryPolicy.linear({ delay: Duration.seconds(2), maxRetries: 7 }) },
      });
